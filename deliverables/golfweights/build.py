...
def product_row(p):
    ...
    return {"page": h1, "url": url, "prio": PRIO[p["prio"]], "title": title, "desc": desc,
            "h1": h1, "h2s": h2s, "avoid": " ".join(avoid), "links": links, "more": [], "block": ""}
...
# ---------------------------------------------------------------- collection rows
def C(page, url, prio, title, desc, h1, h2s, avoid, links, more=None, block=""):
    return {"page": page, "url": url, "prio": prio, "title": title, "desc": desc + " " + HOOKS,
            "h1": h1, "h2s": h2s, "avoid": avoid, "links": links, "more": more or [], "block": block}
...
BRAND_ROWS = [
 C("Scotty Cameron", "/collections/scotty-cameron", "High",
   "Scotty Cameron Golf Weights | Putter Weights, Kits & Tools",
   "Scotty Cameron putter weights, kits and tools — Newport, Phantom, Fastback and more.",
   "Scotty Cameron Golf Weights",
   ["Scotty Cameron Putter Weights", "Scotty Cameron Weight Kits",
    "Scotty Cameron Weight Tools & Wrenches"],
   "Stop targeting 'Scotty Cameron' on its own (currently position 39 — you can't outrank Scotty and the big retailers, and those searchers want a putter, not weights). This page already ranks #2 for 'scotty cameron putter weights' — keep that phrase in the title and first H2 exactly as written.",
   ...
   block="Add as proper H2 headings in a text block: 'Scotty Cameron Putter Weight Chart' (weights by model, 10g–40g, as a real table) and 'How to Change Scotty Cameron Weights' (short steps, link the weight tool). In the intro copy, name the model families — Newport, Phantom, Fastback, Squareback."),
 C("TaylorMade", "/collections/taylormade", "High",
   "TaylorMade Golf Weights | Driver & Spider Putter Weights",
   "TaylorMade driver and Spider putter weights — Qi10, Qi35, Stealth, SIM and BRNR.",
   "TaylorMade Golf Weights",
   ["TaylorMade Driver Weights", "TaylorMade Putter Weights", "TaylorMade Fairway Wood Weights",
    "TaylorMade Weight Kits", "TaylorMade Shaft Adapters", "TaylorMade Wrenches & Tools"],
   "Remove bare model names as targets (e.g. 'TaylorMade Qi10' alone) — model phrases belong on product pages, always with 'weights' added. Keep 'golf weights' brand-qualified (always 'TaylorMade golf weights', never standalone) — this page is one of the six currently competing with the homepage.",
   ...
   block="Optional H3 under the putter section: 'TaylorMade Spider Putter Weights' — the Spider phrase carries most of the putter demand (~740/mo US). In the intro copy, name the ranges — Qi10, Qi35, Stealth, SIM, M-Series, BRNR, Spider, TP."),
 C("Odyssey", "/collections/odyssey", "High",
   "Odyssey Golf Weights | Putter Weights & Weight Kits",
   "Odyssey putter weights and kits — Ai-ONE, Jailbird, White Hot and Stroke Lab.",
   "Odyssey Golf Weights",
   ["Odyssey Putter Weights", "Odyssey Weight Kits", "Odyssey Weight Tools & Wrenches"],
   "This page ALONE owns 'Odyssey putter weights' — keep that phrase in the title and first H2. Two product pages currently compete with it for that phrase and are being retitled to model-only names in this sheet.",
   [HOME, TYPE_HUB["putter"], KITS, ("/collections/golf-club-wrench", "golf torque wrenches"),
   ...
   block="In the intro copy, name the model families — Ai-ONE, Jailbird, White Hot, Stroke Lab, Toulon."),
 C("PXG", "/collections/pxg", "High",
   "PXG Golf Weights | Driver, Iron & Putter Weight Kits",
   "PXG weights and weight kits — Black Ops drivers, Gen-series irons, Battle Ready putters.",
   "PXG Golf Weights",
   ["PXG Driver Weights", "PXG Iron Weights", "PXG Putter Weights",
    "PXG Weight Kits", "PXG Shaft Adapters"],
   ...
   [(TYPE_HUB["putter"])],
   block="Optional H2 in a text block: 'PXG Weight Chart' — there are chart searches for PXG (~80/mo US). In the intro copy, name Black Ops, the Gen series and Battle Ready."),
 C("Callaway", "/collections/callaway", "Medium",
   "Callaway Golf Weights | Driver & Fairway Wood Weights",
   "Aftermarket Callaway driver and fairway wood weights — Elyte, Paradym, Rogue, Mavrik, Epic and Apex.",
   "Callaway Golf Weights",
   ["Callaway Driver Weights", "Callaway Fairway Wood Weights", "Callaway Hybrid Weights",
    "Callaway Weight Kits", "Callaway Shaft Adapters"],
   ...
   block="In the intro copy, name the ranges — Elyte, Paradym, Rogue, Mavrik, Epic, Apex."),
 C("Ping", "/collections/ping", "Medium",
   "Ping Golf Weights | Driver & Iron Weights (G425–G440)",
   "Ping driver weights, iron weight replacements and kits — G425, G430, G440 and more.",
   "Ping Golf Weights",
   ["Ping Driver Weights", "Ping Fairway Wood Weights", "Ping Iron Weights",
    "Ping Putter Weights", "Ping Weight Kits", "Ping Shaft Adapters"],
   "Don't target bare model names ('Ping G425 driver' alone). KEEP the iron weight replacement copy — this page ranks #1 for 'ping iron weight replacement'.",
   ...
   block="Move the existing 'ping iron weight replacement' copy under the Ping Iron Weights section — do not delete or rewrite it (it ranks #1). Optional H2: 'Ping G-Series Weights by Generation (G30–G440)'. Cross-sell the Ping Vault putter wrench in the putter section."),
 C("Titleist", "/collections/titleist", "Medium",
   "Titleist Golf Weights | SureFit Driver Weights & Kits",
   "Titleist SureFit weights and weight kits for GT, TSR, TS and 913–917 drivers and fairways.",
   "Titleist Golf Weights",
   ["Titleist Driver Weights", "Titleist Fairway Wood Weights", "Titleist Hybrid Weights",
    "Titleist Weight Kits", "Titleist Shaft Adapters"],
   "Keep 'golf weights' brand-qualified (always 'Titleist golf weights', never standalone) — this page is one of the six currently competing with the homepage.",
   ...
   block="Optional H2 in a text block: 'Titleist Weight Settings (GT & TS)' — settings searches are big for Titleist (e.g. GT3 weight adjustment, ~200/mo US). Cross-link the SureFit chart on the Titleist adapter page prominently."),
 C("Cobra", "/collections/cobra", "Medium",
   "Cobra Golf Weights | Driver & Putter Weights",
   "Aftermarket Cobra weights — Darkspeed, DS-Adapt, Aerojet, LTDx, Radspeed drivers and King putters.",
   "Cobra Golf Weights",
   ["Cobra Driver Weights", "Cobra Fairway Wood Weights", "Cobra Putter Weights",
    "Cobra Weight Kits", "Cobra Shaft Adapters"],
   ...
   block="In the intro copy, name the ranges — Darkspeed, DS-Adapt, Aerojet, LTDx, Radspeed, King."),
 C("Srixon", "/collections/srixon", "Low",
   ...
   "Srixon Golf Weights",
   ["Srixon Driver Weights", "Srixon Weight Kits", "Srixon Shaft Adapters"],
   "Keep this page light — Srixon's real search demand is shaft adapters.",
   ...
   block="Make the Srixon Shaft Adapters link prominent (banner-level) — adapters are Srixon's biggest search cluster."),
 C("Mizuno", "/collections/mizuno", "Low",
   "Mizuno Golf Weights | ST Driver Weights & Adapters",
   "Aftermarket Mizuno weights and shaft adapters for the ST range of drivers.",
   "Mizuno Golf Weights",
   ["Mizuno Driver Weights", "Mizuno Shaft Adapters"],
   "Keep 'golf weights' brand-qualified (always 'Mizuno golf weights', never standalone) — this page is one of the six currently competing with the homepage.",
   [HOME, TYPE_HUB["driver"], ("/collections/mizuno-shaft-adapters", "Mizuno shaft adapters"),
    ("/products/mz-aftermarket-weights-for-mizuno-st-range-drivers", "Mizuno ST driver weights")],
   block="There IS search demand for 'Mizuno putter weights' (M-Craft, ~10–30/mo UK) but no putter products were found in the catalogue — if you stock M-Craft putter weights, add a 'Mizuno Putter Weights' H2 section and tell us so we can update the targeting."),
 C("LAB Golf", "/collections/lab", "Low",
   "LAB Golf Weights | Putter Weights for DF & Mezz",
   ...
   block="One intro sentence naming DF and Mezz is enough."),
 C("Honma", "/collections/honma", "Low",
   ...
   block="One intro sentence is enough."),
 C("Toulon Design", "/collections/toulondesign", "Low",
   "Toulon Design Golf Weights | Putter Weights & Kits",
   "Aftermarket putter weights for Toulon Design putters, 2022 and 2025 collections.",
   "Toulon Design Golf Weights",
   ["Toulon Putter Weights", "Toulon Weight Kits"],
   "Minimal effort page — negligible search demand. Just apply the title/H1 and move on.",
   [HOME, TYPE_HUB["putter"],
   ...
   block="One intro sentence naming the 2022 and 2025 collections is enough."),
]
...
ADAPTER_ROWS = [
 C("Shaft adapters (hub)", "/collections/shaft-adapters", "High",
   ...
   "Golf Shaft Adapters",
   ["Shop Shaft Adapters by Brand", "Left-Handed Shaft Adapters", "Torque Wrenches for Fitting"],
   "—",
   ...
   block="Add an 'Adapter Settings & Compatibility Charts' H2 section that links to each brand's chart with descriptive anchors ('Titleist SureFit chart', 'Callaway OptiFit chart' etc.)."),
 C("Titleist shaft adapters", "/collections/titleist-shaft-adapters", "High",
   ...
   ["Titleist Driver Adapters", "Titleist Fairway & Hybrid Adapters"],
   "BIGGEST single opportunity in this whole plan — do the chart (see text block column) before anything else on this page.",
   ...
   block="H2 in a text block: 'Titleist SureFit Adapter Chart' — the full A•1–D•4 settings table as REAL TEXT (not an image), plus a which-adapter-fits-which-model list and an FAQ. Searches for this chart alone: ~1,400/mo US, ~100/mo UK, zero competition difficulty."),
 C("TaylorMade shaft adapters", "/collections/golf-shaft-adapters-taylormade", "High",
   ...
   "Do NOT change this page's unusual web address.",
   ...
   block="H2s in a text block: 'TaylorMade Loft Sleeve Compatibility Chart' (which sleeve fits M-Series / SIM / Stealth / Qi generations, as a real table) and 'TaylorMade Adapter Settings Explained'. Your adapter product already ranks #3 for the compatibility chart — lift the chart UP to this page and have the products link to it."),
 C("Callaway shaft adapters", "/collections/callaway-shaft-adapters", "High",
   ...
   ["Callaway Driver Adapters", "Callaway Fairway & Hybrid Adapters"],
   "—",
   ...
   block="H2 in a text block: 'Callaway OptiFit Settings Chart' as a real table — 'callaway adapter settings' is ~500/mo in the US."),
 C("Ping shaft adapters", "/collections/ping-shaft-adapters", "High",
   ...
   ["Ping Driver Adapters", "Ping Fairway & Hybrid Adapters"],
   "—",
   ...
   block="H2 in a text block: 'Ping Adapter Compatibility by Generation' — a table of which adapter (Gen 1/2/3) fits G400 / G410 / G425 / G430 / G440."),
 C("Cobra shaft adapters", "/collections/cobra-shaft-adapters", "Medium",
   ...
   ["Cobra Driver Adapters", "Cobra Fairway Adapters"],
   "—",
   [ADAPT_HUB, ("/collections/cobra", "Cobra golf weights"),
    ("/collections/golf-club-wrench", "golf torque wrenches")],
   block="Light chart section: 'Cobra Adapter Loft Chart' H2 with a settings table."),
 C("PXG shaft adapters", "/collections/pxg-shaft-adapters", "Medium",
   ...
   ["PXG Driver Adapters", "PXG Hybrid Adapters"],
   "—",
   [ADAPT_HUB, ("/collections/pxg", "PXG golf weights"),
    ("/collections/golf-club-wrench", "golf torque wrenches")],
   block="Light chart section: 'PXG Adapter Settings Chart' H2 with a settings table."),
 C("Srixon shaft adapters", "/collections/srixon-shaft-adapters", "Medium",
   ...
   [ADAPT_HUB, ("/collections/srixon", "Srixon golf weights"),
    ("/collections/golf-club-wrench", "golf torque wrenches")],
   block="Modest chart section: 'Srixon Adapter Settings' H2 with a settings table."),
 C("Mizuno shaft adapters", "/collections/mizuno-shaft-adapters", "Low",
   ...
   [ADAPT_HUB, ("/collections/mizuno", "Mizuno golf weights")]),
 ...
 C("Left-handed shaft adapters", "/collections/lh-shaft-adapters", "Medium",
   ...
   block="Short intro plus a per-brand list of what's available left-handed."),
]
